  ...
  return (
    <>
      ...
      <section className="review-layout">
        ...
        <article className="panel review-form"><div className="review-question"><label htmlFor="wins">1. Điều gì đã diễn ra tốt?</label><AutoResizeTextarea id="wins" rows={3} value={draft.wins} onChange={(event) => setDraft((current) => ({ ...current, wins: event.target.value }))} placeholder="Thành tựu, hành động hiệu quả, điều đáng ghi nhận…" /></div><div className="review-question"><label htmlFor="breakdowns">2. Cam kết nào bị bỏ qua và vì sao?</label><AutoResizeTextarea id="breakdowns" rows={3} value={draft.breakdowns} onChange={(event) => setDraft((current) => ({ ...current, breakdowns: event.target.value }))} placeholder="Mô tả sự thật, không đổ lỗi…" /></div><div className="review-question"><label htmlFor="lessons">3. Bạn học được điều gì?</label><AutoResizeTextarea id="lessons" rows={3} value={draft.lessons} onChange={(event) => setDraft((current) => ({ ...current, lessons: event.target.value }))} placeholder="Mô thức, rào cản hoặc điều vừa nhận ra…" /></div><div className="review-question"><label htmlFor="adjustment">4. Tuần sau sẽ điều chỉnh điều gì?</label><AutoResizeTextarea id="adjustment" rows={3} value={draft.adjustment} onChange={(event) => setDraft((current) => ({ ...current, adjustment: event.target.value }))} placeholder="Một thay đổi cụ thể và nằm trong tầm kiểm soát…" /></div><button className="primary-button full" onClick={saveReview}><Save size={17} /> Lưu đánh giá tuần</button></article>
      </section>
