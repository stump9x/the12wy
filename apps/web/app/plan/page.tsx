  ...
  return (
    <>
      ...
      <section className="panel cycle-settings-panel">
        ...
        {showCycleForm && <form className="panel inline-form cycle-create-form" action={addCycle}><div className="form-grid two"><label className="field"><span>Tên chu kỳ mới</span><input name="title" required defaultValue={`12 Week Year · Cycle ${state.cycleHistory.length + 2}`} placeholder="Ví dụ: Ra mắt phiên bản mới" /></label><label className="field"><span>Lý do quan trọng</span><input name="why" placeholder="Điều gì khiến chu kỳ này đáng ưu tiên?" /></label></div><div className="form-actions"><button type="button" className="quiet-button" onClick={() => setShowCycleForm(false)}>Hủy</button><button className="primary-button" type="submit">Thêm chu kỳ</button></div></form>}
        <div className="cycle-settings">
        <div className="form-grid two">
          <label className="field"><span>Tên chu kỳ</span><input value={state.cycle.title} onChange={(event) => updateState((draft) => { draft.cycle.title = event.target.value; })} /></label>
